...
namespace Mimeo 
	...
	public class OrderService
		...
		public void PlaceOrder() 
			...
			// We are just going to use our XML from the Order Quote Reponse
			string PlaceOrderRequestXML = OrderQuoteResponseXML;
			
			// Mimeo Sandbox API - OrderService / PlaceOrder
			string PlaceOrder_URL = "http://connect.sandbox.mimeo.com/2010/09/OrderService/PlaceOrder";	
			
			HttpWebRequest PlaceOrder_Request = (HttpWebRequest) HttpWebRequest.Create (PlaceOrder_URL); 
			
			PlaceOrder_Request.ContentType = "text/xml";
			PlaceOrder_Request.Method = "POST";
			
         		//string s = "id="+Server.UrlEncode(xml);
            		byte[] requestBytes4 = System.Text.Encoding.ASCII.GetBytes(PlaceOrderRequestXML);
            		PlaceOrder_Request.Method = "POST";
            		PlaceOrder_Request.ContentType = "text/xml;charset=utf-8";
            		PlaceOrder_Request.ContentLength = requestBytes4.Length;
            		Stream requestStream4 = PlaceOrder_Request.GetRequestStream();
            		requestStream4.Write(requestBytes4, 0, requestBytes4.Length);
            		requestStream4.Close();
            
			PlaceOrder_Request.Credentials = new NetworkCredential(userName, password);  
			
			string PlaceOrderResponseXML = "";
			using (HttpWebResponse PlaceOrderResponse = PlaceOrder_Request.GetResponse() as HttpWebResponse)  
			{  
			    // Get the response stream  
			    StreamReader reader = new StreamReader(PlaceOrderResponse.GetResponseStream());  
			  
			    // Console application output  
			    PlaceOrderResponseXML = reader.ReadToEnd();
			} 			
			
			// Create our XML Document From New Product Template
			System.Xml.XmlDocument PlaceOrderXML = new XmlDocument();
			PlaceOrderXML.LoadXml(PlaceOrderResponseXML);			
			
			string OrderFriendlyId = PlaceOrderXML.GetElementsByTagName("OrderFriendlyId"].InnerText;
